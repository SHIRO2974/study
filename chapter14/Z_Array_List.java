...
/*         배열 VS 리스트
           1. Array(배열)
           : 여러 데이터를 하나의 object 로 관리하기 위한 자료 구조
           ...
           : Array 의 Index 번호가 데이터의 식별자 역할
                vs List 의 Index 번호는 데이터의 저장 위치만 확인하는 의미
           : 불연속적 메모리 공간을 차지
           : Array 와 달리 동적, 배열의 크기가 정해져있지 않음

            ==============================
            +) List 타입 변수에는 List.of() 와 ArrayList 를 통해
            생성된 불변 리스트, 가변 리스트 모두 할당 가능
            List.of() 메서드
...
*/
        ...
        changeList.add("딸기");
        System.out.println(changeList); // [사과, 바나나, 체리, 딸기]

        List<String> changeList2 = new ArrayList<>();
        changeList2 = List.of("과일1", "과일2");
//      changeList2.add("과일3"); // List of 는 정적 메서드이기 때문에 추가할 수 없다.

/*      *) Array.asList()
           배열을 리스트로 변환하는 메서드
*/
        List<String> asList = Arrays.asList("사과", "바나나");
//      asList.add("수박");
        asList.set(0, "수박");
        System.out.println(asList); // [수박, 바나나]
/*
