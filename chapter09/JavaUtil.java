...
/*      java.util 패키지
        ...
        2. Calendar 클래스
         ...
         - '추상 클래스' 이며 getInstance() 메서드를 통해 구현객체를 반환받아 사용

        3. java.time 패키지
          날짜와 시간을 다루는 도구 (명확성과 유연성을 제공)
         - java.util Date 와 Calendar 문제점을 보완
...
public class JavaUtil {
    public static <SimpleDateFor> void main(String[] args) {
...
//      현재 날짜와 시간: Thu Dec 05 11:33:19 KST 2024
...
//          month(월): 0 ~ 11 까지의 수로 반환
            System.out.println(calendar.get(Calendar.MARCH) + 1);   // 12
            System.out.println(calendar.get(Calendar.DAY_OF_MONTH));    // 5

//          3) set(int year, int month, int , date): 날짜설정
            calendar.set(2025,3,14);
            System.out.println(calendar.get(Calendar.YEAR));    // 2025
            System.out.println(calendar.get(Calendar.MARCH));   // 3
