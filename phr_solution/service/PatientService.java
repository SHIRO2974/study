...
public class PatientService implements PatientServiceInterface {
    ...
    // - 여러 스레드가 동시에 generateId 메서드를 호출하는 것 방지
    ...
    @Override
    public void registerPatient(PatientRequestDto requestDto) { // 환자 등록
        ...
        requestDto.save(newPatient);

    }

    @Override
    public List<PatientResponseDto> listAllPatents() {  // 전체 환자 조회

        List<Patient> patients = repository.findAll();

        List<PatientResponseDto> responseDtos = patients.stream()
                .map(patient -> new PatientResponseDto(patient.getName()
                        , patient.getGender()))
    ...
    @Override
    public PatientResponseDto getPatientById(Long id) { // 단건 환자 조회
    ...
    @Override
    public void updatePatient(Long id, PatientRequestDto requestDto) {  // 환자 정보 수정
    ...
    @Override
    public void deletePatient(Long id) {    // 환자 정보 삭제
