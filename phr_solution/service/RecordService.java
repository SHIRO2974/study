...
public class RecordService implements RecordServiceInterface{
    ...
    @Override
    public void createRecord(RecordRequestDto requestDto) { // 건강 기록 추가
    ...
    @Override
    public List<RecordResponseDto> getAllRecords() {    // 건강 기록 전체 조회
    ...
    @Override
    public List<RecordResponseDto> filterRecordsByDiagnosis(String diagnosis) { // 건강 기록 필터 조회
        List<RecordResponseDto> responseDtos =null;
    ...
    @Override
    public void deleteRecord(Long patientId) { // 건강 기록 삭제

        try {

            HealthRecord healthRecord = recordRepository.findById(patientId)
                    .orElseThrow(() -> new IllegalArgumentException("해당 ID의 건간 기록은 존재하지 않습니다. ID: " + patientId));
