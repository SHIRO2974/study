...
public class BookController {
    ...
    public void start() {
        ...
        while (true) {
            ...
            view.showMenu();
            int choice = view.getInput();   // 숫자값을 검증하여 가져오는 메서드
            ...
            switch (choice) {
                ...
                case 2: // 전체 책 조회: 요청데이터 X, 반환 데이터 O
                    List<Book> books = service.listAllBooks();
                    view.displayBooks(books);
                    break;

                case 3: // 단권 책 조회: 요청 데이터 O, 반환 데이터 O

                    int findBookId = view.getInput();   // 검색하고자 하는 Book 의 ID

                    if (findBookId != -1) {
                        Book findBook = service.findBookById(findBookId);   // 해당 id 로 찾은 Book 객체
