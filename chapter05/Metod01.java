...
/*
       ...
       === 클래스의 메서드 ===
       클래스 내부에서 필드를 사용하거나 특정 작업을 수행하는 코드
      - 클래스 내부에서 정의된 함수
      - 객체가 수행할 수 있는 동작을 정의
      ...
      Return Type methodName(Parameter list) {
        ...
        2. methodName
          메서드를 식별하는 이름
          +) 명명 규칙 - loweCamelCase 동사를 사용하여 메서드의 작업을 설명
          Ex) '먹다' - eat(동사), eating(명사)

       3. Parameter List
          메서드에 전달되는 입력 값들의 목록
         - 0개 이상으로 지정 (지정하지 않을 수 있으며 원하는 개수만큼 전달도 가능)
         - 각 매개변수는 자료형과 함께 선언 (동일한 데이터타입
         ...
         === 함수(Function) VS 메서드(Method) ===
           ...
           2. 메서드
           객체의 동작을 정의한 코드
          - 클래스 내부에 정의되어 객체와 관련됨 ( 함수의 체계를 가진다 )
          - 객체 생성 후 . 연산자를 통해 호출
...
*/
        ...
        int result2 = calculator.addNumBers(50,30);
